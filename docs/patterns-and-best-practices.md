# Minerva Rails Patterns and Best Practices

**A Comprehensive Guide for Rails Developers**

This documentation explains the patterns, conventions, and best practices used in the Minerva Rails application. It's designed for developers who already know Ruby on Rails and need to understand how Minerva structures and organizes code.

## Table of Contents

- [Part 1: Foundation & Architecture](#part-1-foundation--architecture)
  - [1.1 Understanding Minerva's Architecture](#11-understanding-fizzys-architecture)
...
- [Part 5: Coding Style Guide](#part-5-coding-style-guide)
  - [5.1 Minerva Coding Conventions](#51-fizzy-coding-conventions)
...
Before diving into specific patterns, you need to understand Minerva's foundational architecture. These concepts underpin everything else in the application.

## 1.1 Understanding Minerva's Architecture

### Domain Model Overview

Minerva's domain model follows a clear hierarchy:
...
Minerva uses **URL path-based multi-tenancy** rather than subdomains or separate databases:
...
Minerva uses `ActiveSupport::CurrentAttributes` to maintain thread-safe request state:
...
Minerva uses UUIDs (UUIDv7, base36-encoded to 25 characters) instead of auto-incrementing integers:
...
This is where Minerva's distinctive style shines. Models contain business logic and use concerns heavily to organize behavior.

## 2.1 Concern Architecture

Minerva uses concerns extensively to compose model behavior. This is the most distinctive pattern in the codebase.
...
4. Private methods (if needed) - Indented per Minerva style guide
...
Minerva emphasizes method names that read like business domain language. Models provide APIs that express intent clearly.
...
Minerva uses lambda defaults on `belongs_to` associations to automatically propagate context. This reduces boilerplate and enforces multi-tenancy.
...
Scopes in Minerva have descriptive names that express business concepts, not SQL operations. They're composable and chainable.
...
Minerva uses callbacks sparingly. The philosophy: explicit is better than implicit.
...
Minerva has several unique domain-specific features that follow consistent patterns.

## 3.1 Event Tracking System

Events are Minerva's audit trail. Every significant action creates an event record that drives activity timelines, notifications, and webhooks.
...
Minerva tracks file storage usage at the board and account level to enforce quotas. This pattern shows how to handle complex accounting across hierarchies.
...
Minerva's unique "entropy" feature automatically postpones stale cards to prevent infinite todo lists. This shows how to implement complex time-based business rules.
...
Controllers and background jobs in Minerva are extremely thin. They orchestrate, but delegate all business logic to models.

## 4.1 Thin Controllers with Rich Models

Minerva strictly follows the "thin controller, fat model" philosophy. Controllers have 3 responsibilities: setup, call model, respond.
...
**✅ Minerva pattern (model handles logic):**
...
Minerva models all actions as RESTful resources, not custom routes. This is a core pattern from the style guide.
...
Minerva has a strict naming convention for asynchronous operations. Jobs are ultra-thin and delegate everything to models.
...
```ruby
module MinervaActiveJobExtensions
  extend ActiveSupport::Concern
...
ActiveSupport.on_load(:active_job) do
  prepend MinervaActiveJobExtensions  # ← Applied to ALL jobs
end
```
...
Multi-tenancy "just works" in jobs without thinking about it. This is one of Minerva's most powerful patterns.

---

# Part 5: Coding Style Guide

Minerva has specific code style conventions beyond standard Ruby/Rails style.

## 5.1 Minerva Coding Conventions

### Conditional Returns (Expanded Conditionals Preferred)

Minerva prefers expanded conditionals over guard clauses:
...
Step-by-step guides for common development tasks in Minerva.
...
```ruby
resources :cards do
  resource :closure  # ← Minerva pattern
end
```
...
This documentation covers the core patterns and practices used throughout the Minerva Rails application:
...
- **Style**: Minerva-specific conventions for readable code
